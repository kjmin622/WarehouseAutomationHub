...
vector<commands> container_organize()
{
    ...
    for (int i = 0; i <= (container_cnt - 1) / N; i++)
    {
        for (int j = 0; j < M; j++)
        {
            if (i == (container_cnt - 1) / N && (container_cnt - 1) % M < j)
                break;
            ...
            if (copy_space[i][j] == NULL)
            {
                ...
                while (!bfs.empty())
                {
                    ...
                    for (int d = 0; d < 4; d++)
                    {
                        ...
                        stack<int> st = now_st;
                        if (nx < 0 || ny < 0 || nx >= N || ny >= M || nx < i || (nx == i && ny < j) || visited[nx][ny])
                            continue;
...
vector<commands> container_move(int id, position pos)
{
    ...
    while (!bfs.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (visited[next_pos.x][next_pos.y] <= next_value)
                continue;
        ...
        if (grid[nx][ny] != -1)
        {
            ...
            while (!bfs.empty())
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (visited[next_pos.x][next_pos.y] <= next_distance)
                        continue;
...
vector<vector<pair<int, int>>> get_map()
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < M; j++)
        {
            if (space[i][j] != NULL)
            {
                res[i][j].first = space[i][j]->id;
                res[i][j].second = space[i][j]->type_id;
            }
            else
            {
                res[i][j].first = -1;
